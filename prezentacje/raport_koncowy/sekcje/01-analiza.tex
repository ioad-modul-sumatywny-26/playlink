...
WN-07 & Bezpieczeństwo transportu & Szyfrowanie HTTPS + WSS (TLS na brzegu hosta) & \done \\
...
Diagram przypadków użycia przedstawiono na rys.~\ref{fig:uc}. Poniżej, w
tab.~\ref{tab:uc1}--\ref{tab:uc5}, opisano scenariusze pięciu kluczowych przypadków
użycia — po jednym opracowanym przez każdego członka zespołu — w jednolitym formacie
(nazwa, aktorzy, warunki wstępne, przebieg główny i alternatywny).

\begin{table}[H]\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{Opis przypadku użycia UC-1: Rejestracja i logowanie (BIP39). Autor: Nikodem Nowak.}
\label{tab:uc1}
...
Przebieg główny &
  1. Gość wybiera „Enter realm” i generuje 12-słowną frazę BIP39 (klucz derywowany lokalnie, nigdy nie opuszcza przeglądarki). \newline
  2. Klient pobiera jednorazowy \emph{nonce} (\code{POST /auth/request-nonce}). \newline
  3. Klient podpisuje komunikat z nonce (\code{personal_sign}, EIP-191). \newline
  4. Backend odtwarza adres z podpisu, weryfikuje nonce i wydaje token JWT (\code{POST /auth/verify}). \newline
  5. BFF zapisuje JWT w ciasteczku \code{httpOnly}; użytkownik jest zalogowany. \\
Przebieg alternatywny &
  \textbf{4a.} Nonce wygasł lub został już użyty — backend odrzuca weryfikację (HTTP 400/401); klient ponawia od kroku~2. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{Opis przypadku użycia UC-2: Utworzenie pokoju i rozpoczęcie czatu. Autor: Bartosz Kołaciński.}
\label{tab:uc2}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\raggedright\arraybackslash}p{0.26\textwidth}Y@{}}
\toprule
...
Przebieg główny &
  1. Użytkownik wypełnia formularz (nazwa, gra, limit graczy, lokalizacja; opcjonalnie opis, link do komunikatora, wymagania) i zatwierdza. \newline
  2. Backend tworzy pokój (\code{POST /rooms}), dołącza twórcę jako członka i rozgłasza nową listę pokojów na kanale \code{/ws/rooms}. \newline
  3. Użytkownik wchodzi do pokoju; otwiera się uwierzytelniony kanał czatu \code{/ws/rooms/<name>/chat}. \\
Przebieg alternatywny &
  \textbf{1a.} Zajęta nazwa pokoju lub przekroczony limit 3 aktywnych pokojów — backend zwraca błąd walidacji, formularz wyświetla komunikat. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{Opis przypadku użycia UC-3: Dołączenie do pokoju i deklaracja RSVP. Autor: Jakub Rosiak.}
\label{tab:uc3}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\raggedright\arraybackslash}p{0.26\textwidth}Y@{}}
\toprule
...
Przebieg główny &
  1. Użytkownik wybiera pokój z listy i dołącza (\code{POST /rooms/<name>/join}); zaktualizowany roster (\code{roster_update}) jest rozgłaszany do członków. \newline
  2. Jeśli pokój ma zaplanowane wydarzenie, użytkownik deklaruje obecność (\code{present} / \code{maybe} / \code{absent}) przez \code{PUT /rooms/<name>/event/rsvp}; rozgłaszana jest ramka \code{rsvp_update}. \newline
  3. Użytkownik uczestniczy w czacie pokoju. \\
Przebieg alternatywny &
  \textbf{1a.} Pokój jest pełny lub wygasł — dołączenie zostaje odrzucone, użytkownik wraca do listy. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{Opis przypadku użycia UC-4: Moderacja — wyrzucenie uczestnika. Autor: Jakub Rusek.}
\label{tab:uc4}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\raggedright\arraybackslash}p{0.26\textwidth}Y@{}}
\toprule
...
Warunki wstępne & Adres konta znajduje się w \code{ADMIN_ADDRESSES}; cel jest członkiem pokoju \\
Przebieg główny &
  1. Administrator wybiera uczestnika i akcję „kick” (\code{POST /rooms/<name>/members/<address>/kick}). \newline
  2. Backend weryfikuje uprawnienia, usuwa członkostwo i rozgłasza ramki \code{member_kicked} oraz \code{roster_update}. \newline
  3. Połączenia czatu wyrzuconego użytkownika zostają zamknięte (kod \code{4409}). \\
Przebieg alternatywny &
  \textbf{1a.} Żądanie pochodzi od konta bez uprawnień administratora — backend odrzuca operację (HTTP 403). \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]\footnotesize
\renewcommand{\arraystretch}{1.3}
\caption{Opis przypadku użycia UC-5: Wyszukiwanie i filtrowanie pokojów. Autor: Mateusz Kosowski.}
\label{tab:uc5}
\begin{tabularx}{\textwidth}{@{}>{\bfseries\raggedright\arraybackslash}p{0.26\textwidth}Y@{}}
\toprule
Nazwa przypadku & Wyszukiwanie i filtrowanie pokojów \\
\midrule
Aktorzy & Gość lub Użytkownik \\
Warunki wstępne & Brak \\
Przebieg główny &
  1. Aktor otwiera listę pokojów, aktualizowaną na bieżąco przez kanał \code{/ws/rooms}. \newline
  2. Aktor wpisuje frazę wyszukiwania i/lub wybiera filtr gry oraz porządek sortowania. \newline
  3. Lista zawęża się do pokojów spełniających kryteria; wybór pokoju prowadzi do jego szczegółów (a po zalogowaniu — do dołączenia, zob.\ UC-2/UC-3). \\
Przebieg alternatywny &
  \textbf{3a.} Brak pokojów spełniających kryteria — wyświetlany jest stan pusty z podpowiedzią rozszerzenia filtrów lub utworzenia własnego pokoju. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Diagram komponentów (zgodny z \texttt{docker-compose.yml}): przeglądarka,
  usługi \texttt{frontend} (BFF SvelteKit), \texttt{backend} (FastAPI z zadaniem
  czyszczącym) i \texttt{db} (PostgreSQL); wdrożenie przez Tailscale.}
...
\begin{itemize}
  ...
  \item \textbf{WebSocket} — kanał \code{/ws/rooms} (rozgłasza pełną listę pokojów po
        ...
        kontrolą członkostwa; ramki: \code{history}, \code{message}, \code{event_update},
        \code{rsvp_update}, \code{roster_update}, \code{room_closed}, \code{member_kicked},
        \code{error}).
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{klasy.png}
  \caption{Diagram klas PlayLink: encje SQLModel (warstwa danych) oraz menedżery
  połączeń WebSocket \texttt{ConnectionManager} i \texttt{RoomChatManager} (warstwa logiki).}
  \label{fig:klasy}
\end{figure}
